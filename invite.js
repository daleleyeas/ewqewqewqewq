...
client.on("guildMemberAdd", member => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(`${member} Adlı Kişi Sunucuya Katıldı **Davet Eden Şahıs:** ${invite.inviter.tag} (**${total + bonus}** Davet!)`)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`${member.user.tag} Sunucudan Ayrıldı **Şahsı Davet Eden:** ${user.tag} (**${Number(total) + Number(bonus)}** Davet!)`)
     }
});
